...
# st.dataframe(data)
mx_sales, mx_order, mx_customer, mx_prfit_pct = st.columns(4)
...
with mx_order:
    ...
    order_dif_pct = 100.0 * (curr_order - prev_order) / prev_order
    st.metric("Order", value=format_big_number(curr_order), delta=f'{order_dif_pct:.2f}%')
...
with mx_customer:
    ...
    order_cus_pct = 100.0 * (curr_customer - prev_customer) / prev_customer
    st.metric("Customer", value=curr_customer, delta=f'{order_cus_pct:.2f}%')

with mx_prfit_pct:
    curr_provit = data.loc[data['order_year']==CURR_YEAR, 'profit_pct'].values[0]
    prev_provit = data.loc[data['order_year']==PREV_YEAR, 'profit_pct'].values[0]

    order_profit_pct = 100.0 * (curr_provit - prev_provit) / prev_provit
    st.metric("Provit", value=f'{curr_provit:.2f}%', delta=f'{order_profit_pct:.2f}%')
...
st.header('Sales vs Provit Corelation')
